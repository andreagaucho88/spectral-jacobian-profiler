...
input-to-output Jacobian of Transformer language models --- the final-token hidden-state Jacobian at
both final-norm read-out sites, plus an output-distribution susceptibility. From exact Jacobian--vector products it
...
($\mathrm{R}^2\le0.11$, sign unstable), but it compares a sensitivity to a level: against the entropy's
...
\emph{both} coding and arithmetic under clustering. \emph{(iii) The radial reading replicates.} At the
post-norm site factual is no longer the most expansive ($\sigma_{\max}$: coding $88 >$ factual $79 >$
impossible-entity $75 >$ arithmetic $72$; factual vs.\ coding $d=-0.47$), consistent, as on Qwen,
...
impossible-entity ($5256$) $>$ factual ($4840$) $>$ coding ($4447$), and factual is now
significantly \emph{below} arithmetic ($d=-0.67$, Holm $p=0.014$) rather than above it
...
impossible-entity on SmolLM2 ($p=0.000$); arithmetic-involving contrasts are never cluster-robust
...
Post-norm reordering $\Rightarrow$ radial reading & \S\ref{sec:results-alg2} & both primary & consistent-with, not proven \\
...
the load-bearing rows. Our primary contribution is instrumental: a matrix-free spectral profiler for the exact
...
\begin{itemize}\setlength{\itemsep}{3pt}
  ...
  coding ($d=1.12$, cluster permutation $p=0.000$). Not cluster-robust: factual $>$ arithmetic
  ...
  $\nabla_{H_0}H$'s per-position energy profile), which is $0.0075$ (vs.\ the fully-random
  $\sqrt{2/(\pi N)}=0.0046$); $|\!\cos|=0.41$ is still $\approx\!55\times$ this null and $97.5\%$ of
  ...
  correlation is $0.30$ ($0.24,0.25,0.51,0.21$ for factual/coding/arithmetic/impossible-entity; each
