...
This opacity matters most for \emph{hallucination} --- fluent but unsupported output
...
$\sigma_{\max}$ with the bulk norm. Both follow from one structural fact --- the surprisingly low
stable rank ($\approx4$ in $d\approx900$): prompt-conditioned input-to-hidden sensitivity is highly
anisotropic, concentrated in a handful of input directions, which is exactly why a single random
direction estimates the bulk norm rather than the leading amplification, and why the two are so
correlated. \emph{Exploratory} (single-model or small-$n$, pre-specified where
...
explains part of the reshuffle but most of it requires the per-dimension gain $\gamma$. Per-prompt values for all four categories are released
