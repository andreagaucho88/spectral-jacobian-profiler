...
including zero on two of three), while a partialling-free geometric check finds the two input
directions only partially aligned ($|\!\cos\angle(\nabla_{H_0}H,v_{\mathrm{lead}})|=0.41$,
$\sim\!90\times$ chance but $\cos^2\!=\!0.24$). The coupling is thus a shared common scale plus a
modest, genuinely directional residual --- neither independence nor strong coupling. The leading value is also largely redundant with
...
All metrics are Euclidean and representation-dependent.
\end{abstract}
...
side, $\sigma_{\max}$ is strongly correlated with the bulk norm ($r=0.80$, i.e.\ $64\%$ shared variance) for the categorical contrast; the
