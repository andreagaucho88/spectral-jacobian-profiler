...
therefore ask a complementary, \emph{internal} question: do impossible-entity prompts induce
...
to radial removal. Finally, the Fisher/KL susceptibility $\chi_F$ does
...
to the final norm's learned per-dimension gain $\gamma$ and the last block's transformation, not to
...
$\sigma_{\max}$ with the bulk norm. \emph{Exploratory} (single-model or small-$n$, pre-specified where
possible): the geometric-alignment measurement, the specific category orderings, and the
intermediate-scale check. One hypothesis we pre-registered was \emph{refuted} by measurement --- the
...
\begin{itemize}\setlength{\itemsep}{3pt}
  ...
  CI including zero on two of three models) once the bulk norm is removed, the shared driver
  $\lambda_{\mathrm{bulk}}$ is verified from both sides ($0.80$-correlated with $\sigma_{\max}$, $0.50$
  with $\|\nabla_{H_0}H\|$), and a partialling-free geometric check ($|\!\cos|=0.41$, $\approx\!55\times$
...
reshuffle is not a radial-component removal --- it reflects the final norm's learned per-dimension gain
$\gamma$ and the last block's transformation. Per-prompt values for all four categories are released
