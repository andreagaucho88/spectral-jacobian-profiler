...
\title{\bfseries Matrix-Free Spectral Profiling of Prompt-Conditioned Jacobians and Output Susceptibility in Transformer Language Models}
...
Jacobians of Transformer language models --- the final-token hidden-state Jacobian at both final-norm
read-out sites, plus a Fisher/KL susceptibility of the output distribution. From exact Jacobian--vector
products it recovers, per prompt, the leading amplification $\sigma_{\max}$, an unbiased estimate of
the bulk norm $\|J\|_F$, the stable rank, and the output susceptibility $\chi_F$, with a cheap
sampled-direction baseline and a numerical core validated against dense ground truth. We apply it to
four prompt categories (factual, coding, arithmetic, impossible-entity) on Qwen2.5-0.5B,
...
($\mathrm{R}^2\le0.11$, sign unstable), but it compares a sensitivity to a level. The like-for-like
comparison against the entropy's input sensitivity gives $r=0.45$, and this is mostly a shared Jacobian
scale: partialling out the bulk norm leaves a residual partial correlation of only $0.10$--$0.30$ (CI
including zero on two of three models). The category orderings are model- and scale-dependent, and a
trivial linear probe already separates the categories, so the observables characterize response
geometry rather than classify prompts (Results give the redundancy, geometric-alignment and probe
numbers). We release the profiler, the length-matched prompt sets and all per-prompt results. The
lesson: single-direction or level summaries of hidden geometry invite spurious dissociations that
magnitude-controlled comparisons dissolve. All hidden-state metrics are Euclidean and
representation-dependent.
\end{abstract}

\keywords{Transformer language models, prompt-conditioned Jacobian, output susceptibility, matrix-free spectral profiling,
...
This opacity is especially relevant in uncertainty- and \emph{hallucination}-related settings --- fluent but unsupported output
\cite{ji2023survey} --- which output-level analyses (factual correctness, calibration,
truthfulness \cite{lin2022truthfulqa}) can measure but cannot tie to the internal computation. We
therefore study the hidden computation directly, as a \emph{diagnostic geometry of
prompt-conditioned Jacobians}: for a fixed prompt we profile the spectrum of the Jacobian mapping
input perturbations to the final-token hidden state, and ask whether prompts built to induce output
uncertainty (impossible, fictional, or temporally invalid entities --- an uncertainty-inducing
proxy) drive the model through a measurably different perturbation regime than factual-recall or
arithmetic prompts. When we call a trajectory more or less \emph{responsive} we mean a larger or
smaller finite-depth perturbation response, made quantitative below; we claim no chaos and estimate
no Lyapunov spectra, and cross-check the sampled response against the exact Jacobian's leading
direction.
...
hallucinations; the ``impossible-entity'' category is a set of prompts built from impossible,
...
We keep two notions separate \emph{by definition} throughout (Figure~\ref{fig:pipeline} maps both, and
every observable, onto the forward pass) --- whether they are empirically
...
complementary, not interchangeable, and impossible-entity behaviour should not be reduced to
...
nearly uncorrelated with the output-entropy \emph{level} ($\mathrm{R}^2\le0.11$ at every model and scale tested, sign unstable), but this near-zero number is
...
\begin{enumerate}\setlength{\itemsep}{2pt}
  ...
  correlation is a sensitivity-vs-level artifact; the correct like-for-like comparison is coupled but
  mostly shared magnitude, $\sigma_{\max}$ is largely redundant with the bulk norm, and a trivial
  linear probe already separates the categories (all numbers in Section~\ref{sec:controls}). We report
  these couplings and their decomposition, not a ``dissociation.''
\end{enumerate}
...
networks but \emph{profile the exact finite-depth input$\to$final-token Jacobian of a trained model,
...
\caption{\textbf{Where each quantity lives on the forward pass.} A prompt's input embeddings $H_0$ are
mapped by the $L$ Transformer blocks to the final-token hidden state $h_{L,T}$ (\emph{pre}-final-norm,
read-out $-2$), then by the final RMSNorm to the \emph{post}-final-norm state (read-out $-1$), by the
head to logits $z$, and by softmax to the output distribution $p$. \emph{Output-level} observables
(entropy $H(p)$, top-1 $p_{\max}$) are functionals of $p$ alone. The \emph{hidden} observables are
spectral summaries of the input$\to$hidden Jacobian $J=\partial h_{L,T}/\partial H_0$: the leading
amplification $\sigma_{\max}$, the bulk norm $\|J\|_F$ (with the Algorithm-1 sampled-direction
$\lambda_L^{(\epsilon)}$ tracking $\lambda_{\mathrm{bulk}}$), and the stable rank --- read out pre-norm
(final\_index $-2$) and post-norm ($-1$). The Fisher/KL susceptibility $\chi_F$ measures how an input
perturbation moves $p$ (spanning the whole pipeline), and the entropy input-gradient $\nabla_{H_0}H$ is
the like-for-like partner of $\sigma_{\max}$ used in the corrected comparison of
Section~\ref{sec:controls}. ``Pre/post-norm'' always refers to this final-norm read-out, not the
...
$z=W_{\mathrm{out}}\,\mathrm{norm}(h_{L,T})$, where $\mathrm{norm}$ is the model's final normalization
(the pre- vs.\ post-$\mathrm{norm}$ read-out sites of Figure~\ref{fig:pipeline}), and the softmax
$p_i=\exp(z_i)/\sum_j\exp(z_j)$, because it is the single internal coordinate the model exposes.
...
$\lambda_L^{(\epsilon)}$ is a finite-depth, sampled-direction perturbation response. It is
...
the anisotropy identity $\sigma_{\max}^2/(\|J\|_F^2/N)=N/r_{\mathrm{stable}}$ holds to floating point; the $\tfrac12$ factor of the
...
Our two \emph{primary} models are publicly available, instruction-tuned, decoder-only Transformers
(Table~\ref{tab:models}, top two rows) --- Qwen2.5-0.5B-Instruct \cite{qwen2025technical} and
...
France?''), \emph{coding} (structured program synthesis), \emph{arithmetic} (we label it thus because
length matching reduces this constrained-reasoning class to terse unit conversions --- ``6 km in
metres?'' --- as Appendix~\ref{sec:prompt-examples} shows), and \emph{impossible-entity} (a
hallucination-prone \emph{proxy}: nonexistent, fictional, or temporally invalid entities, ``Who was the
president of Atlantis in 1823?''). We use the display names \emph{arithmetic} and
\emph{impossible-entity} throughout the measurements; the internal category keys in the released data
remain \texttt{reasoning} and \texttt{hallucination\_prone}. Prompts are
...
\paragraph{The arithmetic class is quarantined.} At the sample sizes of the exact-Jacobian runs the
arithmetic class is drawn from only five distinct templates (vs.\ $28$--$40$ for the others), and
length matching pushes it toward terse arithmetic (Section~\ref{sec:results-output}). We therefore
\emph{exclude the arithmetic class from all primary, cluster-robust claims}; where it appears in
tables and figures it is shown for completeness only and every arithmetic-involving contrast is
...
\cite{holm1979simple}. All permutation $p$ use the add-one estimator $(b+1)/(B+1)$, so no reported $p$
is literally zero: with $B=10^4$ prompt-level relabelings the raw floor is $\approx\!10^{-4}$, and
after Holm correction over the six pairs the smallest values we report are of order $10^{-3}$;
cluster/block permutations on the template id admit fewer relabelings and are likewise reported no
smaller than $10^{-3}$. For the length control we additionally fit an OLS model of the metric on category
...
arithmetic class, whose lower template diversity compresses its within-category variance. The
...
\subsection{Output uncertainty isolates impossible-entity prompts}
...
impossible-entity prompts have the \emph{highest} output entropy $H(p)$ and the \emph{lowest}
...
impossible-entity is the entropy/top-1 outlier in every model-and-set condition, with large
effect sizes (up to $|d|\approx3.9$ on the un-matched sets). One caveat we state plainly: length
matching alters the arithmetic class toward even terser unit conversions, which on the smaller SmolLM2 inflates
its matched-set entropy \emph{above} the impossible-entity class (arithmetic $3.73$ vs.\
impossible-entity $3.55$; $|d|$ only $0.44$). We therefore read output uncertainty on the un-matched
sets, where impossible-entity is the clean outlier, and use the matched sets for the
hidden-state axis (below). This split of datasets is deliberate, and it is worth stating why it is
\emph{not} dataset-shopping: the output-uncertainty result is close to a \emph{manipulation check} of
how the prompts were built --- impossible-entity prompts have diffuse continuations essentially by
construction (there is no fact to retrieve) --- so it is not a categorical claim we place on equal
footing with the hidden-state geometry, and it should be read on the sets where prompt construction is
undistorted. Length matching, by contrast, is required for the hidden-state axis precisely because it
distorts the arithmetic class (Section~\ref{sec:methods-lengthmatch}); forcing both axes onto one set
would either reintroduce the length confound on the hidden side or corrupt the output-side outlier. We
return to this asymmetry --- output uncertainty as a near-manipulation-check, not a second independent
finding --- in Sections~\ref{sec:controls} and~\ref{sec:discussion}.
...
\emph{largest} finite-depth response, and arithmetic and coding induce a \emph{smaller} response
...
arithmetic in SmolLM, and the impossible-entity class shifts from co-highest with factual in
Qwen ($d=0.09$, n.s.) to clearly below factual in SmolLM ($d=0.99$). We thus report a two-tier
picture rather than a single strict ordering, and read arithmetic-involving contrasts with Cliff's
$\delta$.
...
impossible-entity & $0.0921$ & $-0.0005\,(0.09)$  & $0.0895$ & $-0.0036\,(0.99)$  \\
coding              & $0.0845$ & $-0.0081\,(1.40)$  & $0.0879$ & $-0.0052\,(1.43)$  \\
arithmetic           & $0.0864$ & $-0.0062\,(1.12)$  & $0.0843$ & $-0.0087\,(2.39)$  \\
\bottomrule
\end{tabular}
...
models factual induces the largest response and arithmetic/coding the smallest; all
factual-vs-\{coding,arithmetic\} contrasts have permutation $p<10^{-3}$, whereas factual vs.\
impossible-entity is not significant in Qwen. These are \emph{prompt-level} permutation $p$; as
elsewhere, the arithmetic-involving contrasts are \emph{not} cluster-robust under template-level
permutation (only five arithmetic templates; Section~\ref{sec:methods-data}) and are shown for
...
pre-norm prompts gives Pearson $r=-0.01$ (Spearman $+0.03$; $-0.11$ once the quarantined arithmetic
...
coding $1618$, impossible-entity $1375$, arithmetic $1247$) and the \emph{lowest}-entropy class
...
CI are model-dependent ($R^2\le0.09$ on the two primary models). \emph{(ii) Which pairwise contrast survives
clustering is model-dependent.} On SmolLM2 factual $>$ impossible-entity is cluster-robust
($d=1.40$, cluster permutation $p<10^{-3}$) while factual $>$ coding is not ($d=0.34$, $p=0.19$) --- the
mirror image of Qwen (factual $>$ coding robust, factual vs.\ impossible-entity underpowered),
matching the model-dependence already seen in Algorithm~1. Neither model establishes factual above
\emph{both} coding and arithmetic under clustering. \emph{(iii) The post-norm reordering replicates,
but it is not radial.} At the post-norm site factual is no longer the most expansive ($\sigma_{\max}$:
coding $88 >$ factual $79 >$ impossible-entity $75 >$ arithmetic $72$; factual vs.\ coding $d=-0.47$),
the same reshuffle as on Qwen --- but the radial-component explanation is \emph{refuted} by direct
measurement on Qwen (the leading amplification is tangential and no more radial for factual;
Appendix~\ref{sec:radial}), so we attribute the reshuffle on both models to the final-norm gain, not
to radial removal (the radial test itself was run on Qwen; Appendix~\ref{sec:radial}). Finally, the Fisher/KL susceptibility $\chi_F$ does
...
largest in finite-depth response $\lambda_L^{(\epsilon)}$. Relative to factual, impossible-entity
prompts differ by $d\approx-2.0$ in entropy but by only $d=+0.09$ (Qwen) or $+0.99$ (SmolLM) in
$\lambda_L^{(\epsilon)}$; arithmetic prompts differ by $d\approx-2.0$ in entropy yet by $d=+1.1$
...
impossible-entity prompts are the maximum of hidden amplification (pre-norm, factual prompts are),
...
near-zero correlation ($r=-0.01$; $r=-0.11$ once the quarantined arithmetic class is excluded;
...
evidence of shared magnitude. Consistent with a dominant shared-scale component,
$\lambda_{\mathrm{bulk}}$ is independently correlated with \emph{both} sides: it is
$r=0.80$-correlated with $\sigma_{\max}$ \emph{and}
...
($0.50>0.45$). So the correlations are compatible with a single common scale (big Jacobians are big
everywhere) behind the raw $0.45$; these are correlational data and we do \emph{not} identify
$\lambda_{\mathrm{bulk}}$ as a causal mediator, only report that its removal is what shrinks the
partial. We report this decomposition, not a single number.
...
($N=T\!\cdot\!d\approx3\times10^4$) understates chance; the appropriate comparison is a
\emph{position-matched} null --- random directions carrying $\nabla_{H_0}H$'s per-position energy
profile --- which raises the null only to $0.0075$ (Monte-Carlo and closed form agree). Against it
$|\!\cos|=0.41$ is still $\approx\!55\times$ chance, and $97.5\%$ of prompts exceed their own
...
impossible-entity prompts ($0.55$, consistent with the along-$v_{\mathrm{lead}}$ susceptibility of
...
A logistic probe on the raw final hidden state ($\ell_2$-regularized, $C=1$, features standardized
\emph{within each training fold}; $n=160$, $d=896$, so $n\!\ll\!d$) separates the four categories at
$99.4\%$ under standard 5-fold CV and $98.8\%$ under \emph{template-grouped} CV (StratifiedGroupKFold
on the template id, so no template contributes to both train and test folds; chance $25\%$). The
grouped number is the meaningful one --- near-duplicate templated prompts would otherwise leak across
folds --- and it stays essentially unchanged (per-class recall $0.97$--$1.00$), so the separability is
\emph{not} a template artifact. By contrast, template-grouped probes on our \emph{four spectral
scalars} $(\sigma_{\max},\lambda_{\mathrm{bulk}},r_{\mathrm{stable}},\chi_F^{\max})$ and on
$(\|h_\text{pre}\|,\|h_\text{post}\|,\text{ppl})$ reach only $52\%$ and $72\%$ (ungrouped $59\%$,
$73\%$): the spectral observables carry \emph{some} categorical signal (above the $25\%$ chance) but
far less than the full hidden state --- low-dimensional summaries, not classifiers. Category identity
is trivially decodable from $h_{L,T}$, so we do \emph{not} propose the spectral observables as
...
(greedy) leaves impossible-entity prompts the outlier (impossible-entity-vs-others $|d|=0.9$--$2.1$, vs.\
...
and lowest for coding ($704.3$), with impossible-entity ($865.3$) and arithmetic ($717.3$) between
...
$40$, impossible-entity $28$, arithmetic only $5$). Factual $>$ coding is large and cluster-robust
($d=1.12$; cluster permutation $p<10^{-3}$; cluster-bootstrap $95\%$ CI of the mean difference
$[+133,+297]$). Factual $>$ arithmetic is also large ($d=1.08$) but is \emph{not} cluster-robust
(cluster permutation $p=0.11$), because the arithmetic class is backed by only five templates; we
report it as suggestive, not established. Factual and impossible-entity are statistically
...
coding but --- like every arithmetic-involving contrast --- not cluster-robust against the
low-diversity arithmetic class.
...
$\chi_F^{\max}$ ranks factual \emph{lowest} ($1111$) and impossible-entity \emph{highest} ($2849$),
arithmetic ($2045$) and coding ($1330$) between (Figure~\ref{fig:box}, right). This ordering is
...
and test the category structure on the residuals, the factual-vs-impossible-entity contrast collapses
from $d=-0.98$ to $d=-0.19$ (residual permutation $p=0.46$; Figure~\ref{fig:confound}). The factual/
impossible-entity separation on $\chi_F$ is thus largely a re-expression of their entropy difference.
...
non-entropy structure (a coding-vs-arithmetic contrast). We report it as an output-side,
...
probability; the factual$\leftrightarrow$impossible-entity ordering collapses (Cohen $d$: $-0.98\to-0.19$,
$p=0.46$), while a residual coding/arithmetic contrast remains.}
\label{fig:confound}
\end{figure}

\paragraph{Post-norm: the reordering is \emph{not} radial-component removal.}
At the post-norm read-out (after the final RMSNorm) the hidden axis changes: factual is no longer
the most expansive. The post-norm $\sigma_{\max}$ ordering is arithmetic ($5634$) $>$
impossible-entity ($5256$) $>$ factual ($4840$) $>$ coding ($4447$), factual now
significantly \emph{below} arithmetic ($d=-0.67$, Holm $p=0.014$; Figure~\ref{fig:prepost}). A natural
hypothesis is that factual's excess \emph{pre}-norm amplification lies along the radial direction
$\hat h=h_{L,T}/\|h_{L,T}\|$, which the final RMSNorm removes \cite{zhang2019root}. We \emph{tested}
this directly (Appendix~\ref{sec:radial}) rather than leaving it as ``consistent with''. It fails. With
$u_1$ the leading \emph{left} singular vector, the radial alignment $|\langle u_1,\hat h\rangle|$
averages only $0.09$ (random-direction baseline $\approx0.03$) and the radial fraction of the bulk
$\|\hat h^\top J\|^2/\|J\|_F^2$ is only $\approx1\%$: the leading amplification is overwhelmingly
\emph{tangential}, changing $h$'s direction rather than its norm. And, decisively, neither quantity is
larger for factual than for coding (Cohen's $d=-0.07$ and $-0.09$; Mann--Whitney $p\approx0.4$), so
factual's pre-norm excess is \emph{not} radial. The pre$\to$post reordering is therefore attributable
to the final norm alone --- its per-token RMS rescaling $1/\mathrm{rms}(h)$ and learned gain $\gamma$ --- not to
radial removal --- consistent with the absolute $\sigma_{\max}$ moving in \emph{opposite} directions
across models (pre$\to$post $\sim\!5\times$ up on Qwen, $\sim\!20\times$ down on SmolLM2), a
model-specific gain rather than a shared radial effect. The output response $\chi_F$ is, as expected,
identical between the two sites (it depends on the logits, not the tracked hidden index), so the
output-distribution ordering is norm-invariant, whereas the hidden ``factual is most expansive''
statement holds only pre-norm and is representation-dependent.
...
down on SmolLM2 --- a model-specific final-norm gain; see text). On both,
factual leads only at the pre-norm site and the ordering rearranges post-norm. Direct measurement
(Appendix~\ref{sec:radial}) shows this reshuffle is \emph{not} a radial-component removal --- the
leading amplification is tangential and no more radial for factual --- but the final-norm gain.}
\label{fig:prepost}
\end{figure}

\paragraph{Mechanism (secondary, and also entropy-related).}
The hidden leading direction moves the output more for impossible-entity than for other classes:
$\chi_F$ evaluated along $v_{\mathrm{lead}}$ is highest for impossible-entity (vs.\ factual
$d=-0.86$, coding $d=-0.91$, arithmetic $d=-0.79$). We flag this as secondary rather than a headline,
...
$\alpha$ between the hidden-stretch and output-moving directions is largest for impossible-entity
but significantly so only against coding and arithmetic, not factual ($d=-0.51$, Holm $p=0.077$), and
these arithmetic contrasts are not cluster-robust. The gap is thus driven more by impossible-entity's
...
cluster-robust under template-level permutation. In the ordering columns \emph{fac}/\emph{cod} are
factual/coding, and \emph{rea}/\emph{hal} abbreviate the internal data keys
\texttt{reasoning}/\texttt{hallucination\_prone} (display names arithmetic/impossible-entity). On \emph{both} models factual has the highest
pre-norm $\sigma_{\max}$ and the lowest entropy, but \emph{which} pairwise contrast survives
clustering is model-dependent: factual $>$ coding on Qwen ($p<10^{-3}$) vs.\ factual $>$
impossible-entity on SmolLM2 ($p<10^{-3}$); arithmetic-involving contrasts are never cluster-robust
(only five arithmetic templates), and factual vs.\ impossible-entity on Qwen is underpowered, not
equal. At the post-norm site factual is no longer the most expansive on either model; direct measurement
(Appendix~\ref{sec:radial}) shows this is the final-norm gain, \emph{not} a radial component. The bulk identity replicates (Qwen rel.\ err.\
...
inconsistent across scales (impossible-entity highest at $0.5$B, arithmetic at $1.5$B/$3$B),
...
$-0.01$ at $0.5$B; $\mathrm{R}^2\le0.11$ throughout), and factual remains the \emph{lowest}-entropy
...
magnitude-controlled comparison of Section~\ref{sec:controls} was run at $0.5$B, and the direct radial
test of the pre/post-norm difference (Appendix~\ref{sec:radial}, which \emph{refutes} the radial
reading) is likewise a single-model measurement.
...
\begin{tabular}{@{}p{4.6cm}p{1.15cm}p{2.7cm}p{4.7cm}@{}}
\toprule
\textbf{Claim} & \textbf{Evidence} & \textbf{Scope} & \textbf{Status} \\
\midrule
Weak $\sigma_{\max}$--entropy \emph{level} correlation ($\mathrm{R}^2\le0.11$) & \S\ref{sec:results-inversion},\,\ref{sec:scale} & 5 models, $0.12$--$3$B & scale/arch-robust; sign unstable \\
Like-for-like coupling $r=0.45$, mostly shared magnitude (partial $0.10$--$0.30$) & \S\ref{sec:controls} & Qwen-0.5B; partials replicated $\times3$ & residual CI includes zero on 2/3 \\
Directional alignment $|\!\cos|=0.41$, $\approx\!55\times$ position-matched null & \S\ref{sec:controls} & Qwen-0.5B & single-model \\
factual $>$ coding, pre-norm $\sigma_{\max}$ & \S\ref{sec:results-alg2} & Qwen-0.5B & cluster-robust; \emph{not} general (coding leads at $\ge1.5$B, GPT-2) \\
factual $>$ impossible-entity, pre-norm $\sigma_{\max}$ & \S\ref{sec:results-inversion} & SmolLM2 & cluster-robust; model-dependent mirror \\
Bulk identity (Alg.\,1 $\approx\lambda_{\mathrm{bulk}}$) & \S\ref{sec:results-alg2}, Fig.\,\ref{fig:bulk} & both primary & replicated ($3.3\%$ / $1.9\%$) \\
$\chi_F$ tracks entropy; category ordering collapses on residuals & \S\ref{sec:results-alg2} & both primary (sign flips) & \emph{not} an independent axis \\
Post-norm reordering is \emph{not} radial (leading amplification tangential) & \S\ref{sec:results-alg2}, App.\,\ref{sec:radial} & Qwen-0.5B & radial hypothesis refuted; reordering is the final-norm gain \\
Impossible-entity $=$ entropy outlier (hallucination-prone proxy) & \S\ref{sec:results-output} & all conditions & robust; near-manipulation-check \\
Every arithmetic-involving contrast & passim & 5 templates & never cluster-robust; suggestive \\
\bottomrule
\end{tabular}
\caption{What survives and what does not. Each claim with its evidence, scope, and status --- the map
of exactly how far each result generalizes.}
\label{tab:survives}
\end{table}
...
Table~\ref{tab:survives} maps every claim to its evidence, scope, and status; the prose below expands
the load-bearing rows.

\paragraph{Claim hierarchy.} We grade our claims by evidential strength, and ask readers to weight them
accordingly. \emph{Strong} (method-level, robust across our models): the matrix-free profiler and its
validated numerics, and the level-vs-sensitivity lesson --- that the near-zero $\sigma_{\max}$--entropy
correlation is a sensitivity-vs-level artifact, and that the like-for-like, magnitude-controlled
comparison leaves only a weak residual coupling. \emph{Medium} (replicated on both primary models but
narrower): the bulk identity (Algorithm~1 realizes $\lambda_{\mathrm{bulk}}$) and the redundancy of
$\sigma_{\max}$ with the bulk norm. Both are consistent with one structural fact --- the surprisingly
low stable rank ($\approx4$ in $d\approx900$): prompt-conditioned input-to-hidden sensitivity is highly
anisotropic, concentrated in a handful of input directions. Isotropic random probes read the bulk norm
regardless (the Hutchinson identity is spectrum-independent); the low rank is what makes the leading
and bulk summaries meaningfully distinct yet tightly correlated ($r=0.80$). \emph{Exploratory} (single-model or small-$n$, pre-specified where
possible): the geometric-alignment measurement, the specific category orderings, and the
intermediate-scale check. One hypothesis we pre-specified (with no external registration) was \emph{refuted} by measurement --- the
pre/post-norm reshuffle is not a radial-component removal (Appendix~\ref{sec:radial}) --- which we
report as such. The rest of this section expands the strong and medium tiers.

Our primary contribution is instrumental: a matrix-free spectral profiler for the exact
prompt-conditioned input$\to$final-token Jacobian (plus $\chi_F$), with a cheap baseline, correct cluster-aware inference,
...
($\mathrm{R}^2\le0.11$ at every scale, sign unstable; distance correlation $0.20$), but this is \emph{not} evidence that hidden and output
...
$|\!\cos|=0.41$, $\approx\!55\times$ a position-matched null; Section~\ref{sec:controls}). We therefore do \emph{not} claim
...
Our results do not support the hypothesis that impossible-entity prompts induce the
...
independent axes. The one robust output-side statement is that impossible-entity
...
$\sigma_{\max}$--entropy level correlation persists ($\mathrm{R}^2\le0.11$ at every scale, though its
...
$\ne$ verified hallucination.} The impossible-entity category is a prompt \emph{proxy} built from
...
input-embedding coordinates. We had read the pre/post-norm reversal as \emph{consistent with} a radial
component removed by RMSNorm; a direct test (Appendix~\ref{sec:radial}) \emph{refutes} that --- the
leading amplification is tangential --- so we attribute the reversal to the final-norm gain, not a
radial decomposition. \emph{(5)~Not a Lyapunov exponent.}
...
arithmetic class toward even terser unit conversions, inflating its output entropy on small models; we mitigate
...
cluster-robustness.} The matched sets draw the arithmetic class from only five templates (vs.\
$28$--$40$ for the others); consequently the factual-vs-arithmetic $\sigma_{\max}$ contrast and every
arithmetic-involving separation are \emph{not} cluster-robust (cluster permutation $p=0.11$), and we
...
the non-significant contrasts (factual vs.\ impossible-entity, coding vs.\ arithmetic) are
...
should be completed across both models and the post-norm read-out (Table~\ref{tab:alg2}). Our
radial test (Appendix~\ref{sec:radial}) was run only on Qwen; repeating it on SmolLM2 would confirm
that the pre/post reshuffle is the final-norm gain there too. The per-direction distribution's upper tail
...
subset. The examples make two design facts visible: the \emph{arithmetic} class collapses onto a handful
of arithmetic templates (why we quarantine it from primary claims), and the
\emph{impossible-entity} class is a \emph{hallucination-prone proxy} --- fictional or nonexistent
...
arithmetic           &  5 & ``6 km in metres?'';\; ``5 km in metres?'';\; ``2 km in metres?'' \\
impossible-entity & 28 & ``Capital of Westeros?'';\; ``Airport code of Atlantis?'';\; ``Constitutional article of Atlantis?'' \\
\bottomrule
\end{tabular}
\caption{Representative prompts per category, with the count of distinct templates in the
$n{=}40$/category exact-Jacobian subset (the low-diversity \emph{arithmetic} class has only five). All
...
\begin{itemize}\setlength{\itemsep}{3pt}
  ...
  near-duplicates; distinct templates per category are factual $39$, coding $40$, impossible-entity
  $28$, arithmetic only $5$. We therefore judge the primary contrasts by \emph{cluster} bootstrap and
  ...
  coding ($d=1.12$, cluster permutation $p<10^{-3}$). Not cluster-robust: factual $>$ arithmetic
  ($p=0.11$) and every arithmetic-involving contrast, given only five arithmetic templates --- reported
  as suggestive.
  \item \textbf{Equivalence testing.} Non-significant contrasts (factual vs.\ impossible-entity,
  coding vs.\ arithmetic) are \emph{not} treated as equivalences: TOST at a $\pm0.5$\,SD margin is
  ...
  susceptibility is independent of output entropy. It is not ($r=0.49$; the factual/impossible-entity
  ...
  \item \textbf{The coupling is magnitude-dominated (the correct comparison).} Full derivation and
  per-model numbers are in Section~\ref{sec:controls}; in brief, the like-for-like
  $r(\|\nabla_{H_0}H\|,\sigma_{\max})=0.45$ drops to a partial correlation of $0.10$--$0.30$ (residual
  CI including zero on two of three models) once the bulk norm is removed, $\lambda_{\mathrm{bulk}}$
  is independently correlated with both sides ($0.80$ with $\sigma_{\max}$, $0.50$ with $\|\nabla_{H_0}H\|$),
  consistent with a dominant shared scale (no causal identification), and a partialling-free geometric check ($|\!\cos|=0.41$, $\approx\!55\times$
  a position-matched null, but $\overline{\cos^2}=0.24$) leaves only a modest, genuinely directional
  residual. One appendix-only detail: a weak \emph{nonlinear} dependence that Pearson misses (pooled
  distance correlation $0.20$, permutation $p=0.04$) is \emph{not} merely centroid separation --- the
  within-category mean is $0.30$ ($n=40$/class, upward-biased) --- so it persists inside each class
  while staying weak.
  ...
  differs between them (factual leads only pre-norm); a direct test (Appendix~\ref{sec:radial})
  \emph{refutes} a radial-component reading --- the leading amplification is tangential --- so this is
  the final-norm gain. The output-distribution ordering $\chi_F^{\max}$ is identical at both
...
\section{Radial vs.\ Tangential Decomposition (a pre-specified test of the radial reading)}
\label{sec:radial}
The pre/post-norm reshuffle (Section~\ref{sec:results-alg2}) suggested that factual's excess pre-norm
amplification lies along the \emph{radial} direction $\hat h=h_{L,T}/\|h_{L,T}\|$, which the final
RMSNorm removes. We test this directly on the Qwen $n=40$/category prompts. From Algorithm~2 we already
have the leading right singular vector $v_1$ and $\sigma_{\max}$; the leading \emph{left} singular
vector is $u_1=Jv_1/\sigma_{\max}$ (one extra JVP), and the radial row of the Jacobian is
$\hat h^\top J=\nabla_{H_0}\|h_{L,T}\|$ (one VJP). We report (i) the radial alignment of the leading
direction $|\langle u_1,\hat h\rangle|$ and (ii) the radial fraction of the bulk
$\rho=\|\hat h^\top J\|^2/\|J\|_F^2$. \textbf{Pre-specified prediction} (stated before running):
factual has higher $|\langle u_1,\hat h\rangle|$ and $\rho$ than coding.

\textbf{Outcome: refuted.} The leading amplification is overwhelmingly \emph{tangential}:
$|\langle u_1,\hat h\rangle|$ averages $0.09$ across categories (random-direction baseline
$1/\sqrt{N}\!\approx\!0.03$) and $\rho\approx0.011$ (baseline $1/N\!\approx\!0.001$), i.e.\ only about
$1\%$ of the Jacobian's energy changes the norm of $h$; the rest rotates its direction. And the
category contrast goes the \emph{wrong} way for the prediction: factual is not above coding on either
measure ($|\langle u_1,\hat h\rangle|$ factual $0.086$ vs.\ coding $0.091$, Cohen's $d=-0.07$,
Mann--Whitney $p=0.41$; $\rho$ factual $0.011$ vs.\ coding $0.012$, $d=-0.09$, $p=0.45$; both
$n=40$). Factual's pre-norm $\sigma_{\max}$ excess is therefore \emph{not} radial, so the pre/post
reshuffle is not a radial-component removal --- it reflects the final normalization alone (the only operation between the two read-out sites): its
per-token RMS rescaling $1/\mathrm{rms}(h)$ and its learned per-dimension gain $\gamma$. Regressing
$\sigma_{\max}^{\mathrm{post}}$ on the pure-rescaling prediction $\sigma_{\max}^{\mathrm{pre}}/\mathrm{rms}(h)$
gives $\mathrm{R}^2=0.31$ (vs.\ $0.22$ for $\sigma_{\max}^{\mathrm{pre}}$ alone), so scalar RMS rescaling
explains part of the reshuffle but most of it requires the per-dimension gain $\gamma$. Strictly, the
RMSNorm Jacobian also contains the derivative of the rms denominator (a projection along $\hat h$);
given the measured tangentiality of the leading direction ($|\langle u_1,\hat h\rangle|\approx0.09$),
this term is negligible for the leading amplification, so the residual beyond scalar rescaling is
attributable to $\gamma$. Per-prompt values for all four categories are released
(\texttt{results\_analysis/radial\_test.csv}).
...
distance correlation, the template-grouped linear probe, multi-token entropy, the entropy-gradient
partial-correlation and bulk-driver test, the $v_{\mathrm{lead}}$ geometric-alignment test with its
position-matched null, and the extracted $d{=}896$ hidden states as an \texttt{.npz}) are released as self-contained
...
git tag \texttt{v1.0} marks the version reported here.
