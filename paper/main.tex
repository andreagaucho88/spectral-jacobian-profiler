...
\title{\bfseries Matrix-Free Spectral Profiling of Prompt-Conditioned Input$\to$Output Jacobians in Transformer Language Models}

\author{%
  Andrea Gaudiello\,\orcidlink{0000-0001-7721-8217}, Ph.D., MBA\\
  Leonardo Hypercomputing Continuum (LHyC)\\
  Deep-Stack Architecture -- DevOps \& Middleware\\
  Leonardo S.p.A., Via Pieragostini 80, 16151 Genova, Italy\\
  \texttt{andrea.gaudiello@leonardo.com}
}
\date{\today}
...
\begin{abstract}
We introduce a matrix-free toolkit for \emph{offline spectral profiling} of the prompt-conditioned
geometry of Transformer inference: from exact input$\to$final-token Jacobian--vector products we
recover, per prompt, the leading amplification $\sigma_{\max}$, an unbiased estimator of the bulk
norm $\|J\|_F$, and a Fisher/KL output susceptibility $\chi_F$, with a cheap sampled-direction
baseline and a numerical
core validated against dense ground truth. We demonstrate the toolkit on a prompt-category case study
under exact token-length matching and cluster-aware, prompt-level inference, and --- rather than sell
a headline --- we use it to \emph{measure the couplings honestly}. The leading hidden amplification
$\sigma_{\max}$ is only weakly related to output-entropy \emph{level} (Pearson $|r|\le0.11$), but that
near-zero number compares a sensitivity to a level; the like-for-like comparison against the entropy's
\emph{input sensitivity} $\|\nabla_{H_0}H\|$ gives $r=0.45$, which is \emph{mostly shared Jacobian
magnitude} --- partialling out the bulk norm drops it to $0.10$--$0.30$ across models (residual CI
including zero on two of three). So hidden and output response magnitudes co-vary through a common
scale, leaving only a weak, model-dependent residual coupling. The leading value is also largely redundant with
the bulk norm for the categorical contrast ($r(\sigma_{\max},\lambda_{\mathrm{bulk}})=0.80$, expected
from stable rank $\approx4$), and a trivial linear probe separates the categories at $99\%$, so the
observables are characterizations of response geometry, not classifiers. The category orderings are
model- and scale-dependent (on GPT-2 and Qwen-1.5B/3B a different class leads), and $\chi_F$ partially
tracks entropy with an analytic skeleton; we therefore report the method as an \emph{offline
diagnostic}, not a general finding. Our positive contribution is the profiler and the controlled
methodology; the substantive lesson is negative and worth stating: single-direction/level summaries
of hidden geometry are easy to over-read, and the correct like-for-like comparisons must be made.
All metrics are Euclidean and representation-dependent, and the sampled-direction statistic is not a
Lyapunov exponent.
\end{abstract}
...
nearly uncorrelated with the output-entropy \emph{level} ($|r|\le0.11$), but this near-zero number is
...
\begin{enumerate}\setlength{\itemsep}{2pt}
  ...
  is $80\%$ redundant with the bulk norm for the categorical contrast; and a trivial linear probe
...
particles \cite{geshkovski2023emergence} bound and describe this evolution. Our contribution is
...
estimator \cite{hutchinson1990stochastic}. By the same isotropy $\|Jv\|$ concentrates around
$\|J\|_F/\sqrt n$ with relative fluctuations $O(n^{-1/2})$. With $n\sim10^4$ the
single-random-direction statistic of Algorithm~1 is therefore a nearly prompt-invariant
\emph{bulk} quantity, and its normalized logarithm should track
$\lambda_{\mathrm{bulk}}=\tfrac{1}{2L}\log(\|J\|_F^2/n)$. We estimate $\|J\|_F^2$ by the
...
(Table~\ref{tab:models}), chosen to differ in scale and pre-training while sharing modern
...
\begin{tabular}{lrrr}
\toprule
Model & Parameters & Blocks $L$ & Hidden $d$ \\
\midrule
Qwen2.5-0.5B-Instruct & $\approx0.49$\,B & 24 & 896 \\
SmolLM2-360M-Instruct & $\approx0.36$\,B & 32 & 960 \\
\bottomrule
\end{tabular}
\caption{Models used; $L$ and $d$ are read at run time from \texttt{model.config}.}
\label{tab:models}
\end{table}
...
The categories differ systematically in token length, and because the input perturbation has
unit Frobenius norm over the whole $T\times d$ sequence its per-token magnitude scales as
...
length-matched sets we re-derive here (both models, $n=200$/category for the output observables),
...
hallucination-prone is not significant in Qwen.}
...
honest qualifications follow. \emph{(i) The weak level-correlation is not even sign-stable.} On
...
``independent axes.'' It is not, and the honest analysis is instructive --- it is the main lesson of
...
$|\!\cos\angle(\nabla_{H_0}H,v_{\mathrm{lead}})|=0.41$ (median $0.37$), far above the random-vector
baseline $\sqrt{2/(\pi N)}\approx0.004$ for $N=T\!\cdot\!d\approx3\times10^4$ --- roughly $90\times$
chance. So the two directions are \emph{not} orthogonal: the input direction that most stretches the
hidden state carries a genuine component of the entropy sensitivity. But the alignment is partial, not
dominant ($\overline{\cos^2}=0.24$: about three-quarters of the entropy-sensitivity direction lies
\emph{outside} the leading stretch direction), it is largest for hallucination-prone prompts
($0.55$, consistent with the along-$v_{\mathrm{lead}}$ susceptibility of
Section~\ref{sec:results-alg2} and, like it, partly entropy-related), and part of even this overlap is
shared token-position support --- both quantities concentrate on the last few positions --- which a
position-whitened metric would discount (future work). The two analyses agree on an intermediate
...
A logistic probe on the raw final hidden state separates the four categories at $99\%$ (5-fold CV;
chance $25\%$); a probe on the three scalars $(\|h_\text{pre}\|,\|h_\text{post}\|,\text{ppl})$ reaches
only $66\%$. Category identity is thus trivially decodable, and we do \emph{not} propose the spectral
observables as classifiers. Their content is the \emph{correlation structure} between response
geometry and output uncertainty --- the level-vs-sensitivity and magnitude decomposition above ---
which a supervised category probe does not provide.
...
profiling at both read-out sites on Qwen; the SmolLM2 pre-norm profiling (which shows the same
weak $\sigma_{\max}$--entropy-level pattern) is reported in Section~\ref{sec:results-inversion}, and SmolLM2 post-norm and
the intermediate-scale sanity check (Section~\ref{sec:scale}) are offline follow-ups. The
...
moderately coupled to the entropy's input \emph{sensitivity} (Section~\ref{sec:controls}); the honest
...
$3.3\%$, $r=0.79$; SmolLM2 $1.9\%$, $r=0.88$). The output ordering $\chi_F^{\max}$ is
...
two intermediate-scale models, Qwen2.5-1.5B and Qwen2.5-3B, as an offline follow-up to the main
...
\emph{LayerNorm}, a base non-instruct model; $n=16$/category), verifying that final\_index $=-1$ is the
...
($|r|\le0.11$, distance correlation $0.20$), but this is \emph{not} evidence that hidden and output
...
$|\!\cos|=0.41$, $\sim\!90\times$ chance; Section~\ref{sec:controls}). We therefore do \emph{not} claim
...
The case study's lesson is methodological, not a dissociation. Measured naively, hidden amplification
$\sigma_{\max}$ looks unrelated to output entropy; but that compares a sensitivity to a level, and the
corrected sensitivity-to-sensitivity comparison is coupled ($r=0.45$), mostly through shared
Jacobian magnitude (partial correlation $0.10$--$0.30$; on two of three models the residual
$95\%$ CI includes zero; Section~\ref{sec:controls}). We therefore
\emph{do not} claim two independent axes. The one robust output-side statement is that
impossible/fictional/hallucination-prone prompts are the entropy outlier --- a near-manipulation-check
(such prompts have diffuse continuations by construction), not a hidden-state finding. On the hidden
side, $\sigma_{\max}$ is $80\%$ redundant with the bulk norm for the categorical contrast; the
Fisher/KL $\chi_F$ tracks entropy with an analytic reason and flips sign across models; and which
class leads $\sigma_{\max}$ is model-specific (factual only on the two sub-billion instruction-tuned
models; coding on GPT-2 and $\ge1.5$B). We are also careful about what this does \emph{not} say: we do
not detect hallucinations (we never evaluate answers; the category is a prompt proxy), and
$\lambda_L^{(\epsilon)}$ is a finite-depth response, not evidence of chaotic dynamics. What we do
offer is the profiler and the discipline of measuring these couplings correctly --- and a concrete
...
Because our primary models are sub-billion, we pre-specified (Section~\ref{sec:scale}) three readings of the
intermediate-scale check (Section~\ref{sec:scale}). The completed check ($1.5$B and $3$B, $n=8$/cat)
supports a \emph{split} of readings (i) and (ii), which we state honestly, and with an important
...
general law. This is the honest position: only the coarse, naive level statistic is scale-stable in
...
completed for Qwen2.5-0.5B (both read-out sites) and SmolLM2-360M (pre-norm), and the
intermediate-scale check ($1.5$B/$3$B) uses only $n=8$/category. Only the \emph{naive} level
...
\begin{itemize}\setlength{\itemsep}{3pt}
  ...
  $T\times d$ space) have mean $|\!\cos|=0.41$ (Qwen, $n=160$), $\sim\!90\times$ the random baseline
  $\sqrt{2/(\pi N)}$ but only $\overline{\cos^2}=0.24$, so the directions are partially aligned, not
  orthogonal and not collinear (partly shared token-position support). There is also a
  ...
  \item \textbf{Scale sanity check.} A deliberately minimal check on Qwen2.5-1.5B/3B
...
under the MIT license at \url{https://github.com/andreagaucho88/spectral-jacobian-profiler}.
